...
def async_class(cls):
    """Class decorator that applies `run_sync_in_thread_running_loop` to all synchronous methods."""
    for attr_name, attr_value in cls.__dict__.items():
        if callable(attr_value) and not asyncio.iscoroutinefunction(attr_value) and not attr_name.startswith("__"):
            setattr(cls, attr_name, run_sync_in_thread_running_loop(attr_value))
    return cls
